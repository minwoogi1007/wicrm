...
@Getter
@Setter
public class WeeklySummaryStatsDto {
    private String statDate; // 통계 기준일 (주간의 경우, 해당 주의 시작일 또는 특정 대표일)
    ...
    // 기본 생성자
    public WeeklySummaryStatsDto() {
    }

    // 모든 필드를 포함하는 생성자 (필요에 따라 추가)
    public WeeklySummaryStatsDto(String statDate, long totalCalls, long answeredCalls, long abandonedCalls, long uniqueInboundCalls, long busyCalls, long completedCounselingCalls, Double answerRate, Double completionRate) {
